...
class CustomBertLSTMModel(nn.Module):
    ...
    def forward(self, sents):
        ...
        #encoded_layers, pooled_output = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)
        #for xlnet
        encoded_layers_t = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)
        print(encoded_layers_t)
        encoded_layers = encoded_layers_t[0]
        #L1=list(encoded_layers_tuple)
        #L1.pop(0)
        #encoded_layers = tuple(L1)
        #encoded_layers = torch.stack(encoded_layers_t, 1)
        #print(encoded_layers)
        #encoded_layers = encoded_layers.permute(1, 0, 2)
        enc_hiddens, (last_hidden, last_cell) = self.lstm(pack_padded_sequence(encoded_layers,))
...
class CustomBertLSTMAttentionModel(nn.Module):
    ...
    def forward(self, sents):
        ...
        sents_tensor, masks_tensor, sents_lengths = sents_to_tensor(self.tokenizer, sents, self.device)
        #for reoberta and bert
        #encoded_layers, pooled_outputs = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)
        #for xlnet
        encoded_layers = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)
        encoded_layers = encoded_layers.permute(1, 0, 2)

        enc_hiddens, (last_hidden, last_cell) = self.lstm(pack_padded_sequence(encoded_layers, sents_lengths,enforce_sorted = False))
        output_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (batch_size, 2*hidden_size)
        output_hidden = self.dropout(output_hidden)
        pre_softmax = self.hidden_to_softmax(output_hidden)
...
class CustomBertConvModel(nn.Module):
    ...
    def forward(self, sents):
        ...
        sents_tensor, masks_tensor, sents_lengths = sents_to_tensor(self.tokenizer, sents, self.device)
        #for roberta
        #_ , pooled_outputs, encoded_layers = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)
        #for xlnet
        #print(encoded_layers)
        _ ,encoded_layers = self.bert(input_ids=sents_tensor, attention_mask=masks_tensor)

        L1=list(encoded_layers)
        L1.pop(0)
        encoded_layers = tuple(L1)
