...
def validation(model, df_val, bert_size, loss_func, device):
    ...
    with torch.no_grad():
        for i in range(n_batch):
            ...
            targets = torch.tensor(InformationType_label[i*val_batch_size: (i+1)*val_batch_size], dtype=torch.long, device=device)
...
def train(args):
    ...
    if args['MODEL'] == 'default':
        ...
        optimizer = AdamW(parameters, lr=float(args['--lr-bert']), correct_bias=False)
        scheduler =get_linear_schedule_with_warmup(optimizer=optimizer,num_warmup_steps=100, num_training_steps=1000)
...
def test(args):
    ...
    with torch.no_grad():
        for i in range(n_batch):
            ...
            targets = torch.tensor(InformationType_label[i * test_batch_size: (i + 1) * test_batch_size],dtype=torch.long, device=device)
            batch_size = len(sents)
